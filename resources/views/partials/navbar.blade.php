<header style="position:fixed;top:0;left:0;right:0;z-index:50;background:#0F0F0F;">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between" style="height:64px;">
        <a href="{{ route('home') }}" class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-full gold-bg flex items-center justify-center"><i class="fas fa-gem text-sm" style="color:#0F0F0F"></i></div>
        ...
        <nav class="hidden lg:flex items-center gap-8">
            ...
            <a href="{{ route('orders.index') }}" class="text-sm font-medium hover:text-gold" style="color:#9A9590">Pesanan Saya</a>
        </nav>
        <div class="flex items-center gap-3">
            <a href="{{ route('cart.index') }}" class="relative p-2 hover:text-gold" style="color:#9A9590">
                <i class="fas fa-shopping-bag text-lg"></i>
                @php $cc = array_sum(session('cart', [])); @endphp
                @if($cc > 0)
                    <span style="position:absolute;top:-4px;right:-4px;min-width:20px;height:20px;background:#C8A951;color:#0F0F0F;font-size:11px;font-weight:700;border-radius:9999px;display:flex;align-items:center;justify-content:center;padding:0 4px;">{{ $cc }}</span>
                @endif
            </a>
            @if(auth()->check())
    ...
    <div id="mobile-menu" class="hidden lg:hidden" style="border-top:1px solid #2A2A2A;background:#1A1A1A;">
        <div class="px-4 py-4 flex flex-col gap-3">
            ...
            <a href="{{ route('orders.index') }}" class="text-sm hover:text-gold py-2" style="color:#9A9590">Pesanan Saya</a>
            @if(auth()->check())
