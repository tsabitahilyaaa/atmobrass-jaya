...
@section('title', 'Prediksi XGBoost')

@section('content')
<h1 class="font-display font-bold text-2xl mb-3">Prediksi Produksi XGBoost</h1>
...
<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
	...
	<form method="POST" action="{{ route('admin.xgboost.reload') }}">
		@csrf
...
<div class="bg-dark-100 border border-dark-300 rounded-xl p-6 mb-8">
	<p class="text-sm text-muted mb-3">Halaman ini menampilkan hasil prediksi produksi dari model XGBoost yang berjalan di backend Python.</p>
...
<div class="bg-dark-100 border border-dark-300 rounded-xl p-6 mb-8">
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
		<div>
			<h2 class="font-semibold text-lg">Grafik Prediksi Produksi XGBoost</h2>
	...
	<div style="position:relative;height:360px;">
		<canvas id="xgboostPredictionChart"></canvas>
	</div>
</div>
@endif
...
<script>
	document.addEventListener('DOMContentLoaded', function () {
		@if($predictions->count() > 0)
			...
			const canvas = document.getElementById('xgboostPredictionChart');
			if (canvas) {
