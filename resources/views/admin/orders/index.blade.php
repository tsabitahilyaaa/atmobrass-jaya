...
<div class="flex flex-wrap gap-2 mb-6">
    <a href="{{ route('admin.orders.index') }}"
       class="px-4 py-2 rounded-full text-xs font-medium transition-all {{ !request('status') ? 'btn-gold' : 'bg-dark-100 border border-dark-300 text-muted hover:border-gold-dark' }}">
        Semua
    </a>

    @foreach($statusList as $value => $label)
        <a href="{{ route('admin.orders.index', ['status' => $value]) }}"
           class="px-4 py-2 rounded-full text-xs font-medium transition-all {{ request('status') === $value ? 'btn-gold' : 'bg-dark-100 border border-dark-300 text-muted hover:border-gold-dark' }}">
            {{ $label }}
        </a>
    @endforeach
</div>
...
<a href="{{ route('admin.orders.show', $order->id) }}"
   class="block bg-dark-100 border border-dark-300 rounded-xl p-5 hover:shadow-lg transition-shadow">

    <div class="flex flex-wrap justify-between items-start gap-3 mb-3">

        <div>
            <p class="text-xs text-muted">
                {{ $order->order_number }} — {{ $order->formatted_date }}
            </p>
        ...
        <div class="flex items-center gap-3">
            ...
            <form method="POST"
                ...
                <select
                    ...
                    class="input-dark px-3 py-1.5 rounded-lg text-xs">
    ...
    <div class="flex flex-wrap gap-2 mb-3">
            @foreach($order->items as $item)
                <span class="text-xs bg-dark-200 px-3 py-1 rounded-full">
        ...
        <span>
            <i class="fas fa-phone mr-1 text-gold"></i>
            {{ $order->shipping_phone }}
        </span>

        <span>
            <i class="fas fa-credit-card mr-1 text-gold"></i>
            {{ strtoupper($order->payment_method) }}
        </span>

        @if($order->payment_amount)
            <span>
                <i class="fas fa-money-bill-wave mr-1 text-gold"></i>
